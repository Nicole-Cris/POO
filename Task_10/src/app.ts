...
export class App {
    ...
    async removeUser(email: string): Promise<void> {
        ...
        // Verifique se existem aluguéis em aberto para o usuário
        const openRents = await this.rentRepo.findOpenRentsFor(email);
        
        if (openRents.length > 0) {
            // Lança uma exceção se houver aluguéis em aberto
            throw new Error("Usuário possui aluguéis em aberto e não pode ser removido.");
        }
        
        // Se não houver aluguéis em aberto, remova o usuário
